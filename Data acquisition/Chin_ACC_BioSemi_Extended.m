...
% ntrials = 40;
    ...
    %AB = shift up, AC = shift down
    shift = 5; %shift in percent
    alt = 1;
    noise_on = 0;
    db_flank = 64;
    db_main = 70;
    ...
    F0_A = 250;
    F0_B = F0_A*(1+shift/100);
    F0_C = F0_A*(1-shift/100);
    ...
    ncycles_A = round(dur_A*F0_A);
    ncycles_B = round(dur_B*F0_B);
    ncycles_C = round(dur_C*F0_C);
    
    [x_AB, ~] = makeACCStim_swept(F0_A,F0_B,dur_A, dur_B, alt, noise_on, db_flank, db_main, sigrms, rank, nharms, ramp, t_transit, fs);
    [x_AC, ~] = makeACCStim_swept(F0_A,F0_C,dur_A, dur_C, alt, noise_on, db_flank, db_main, sigrms, rank, nharms, ramp, t_transit, fs);
    
    x_AB = scaleSound(x_AB);
    x_AC = scaleSound(x_AC);
    
    % Setup triggers 1,3 = low, 2,4 = high, upper one is pos polarity
